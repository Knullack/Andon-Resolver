...
class resolver:
    ...
    def login(self) -> None:
        entry = self.driver.find_elements('xpath', '/html/body/div/div/div/awsui-app-layout/div/main/div/div[2]/div/span/div/h1')
        if entry:
            self.driver.get(self.FCMenu_URL)
            input_element = self.driver.find_element('xpath', '//*[@id="badgeBarcodeId"]')
            self.type_and_click(input_element, self.badge)
        else: print('Failed to get input element for badge')
